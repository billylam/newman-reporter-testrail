      ...
      if (execution.assertions) {
        execution.assertions.forEach((assertion) => {
          ...
          for (let i = 0; i < strings.length; i++) {
            ...
            if (matches) { // Test case found
              const currentResult = {
                case_id: matches[1],
              };

              // Parse URL from postman object
              const urlObject = execution.request.url;
              let url = `${urlObject.protocol}://${urlObject.host.join('.')}${urlObject.path ? `/${urlObject.path.join('/')}` : ''}`;
              if (urlObject.query.members && urlObject.query.members.length) {
                const queryString = `?${urlObject.query.members.map((param) => `${param.key}=${param.value}`).join('&')}`;
              ...
              if (assertion.skipped === true) {
                ...
                if (assertion.error) {
                  ...
                  if (this.results.map((result) => result.case_id).includes(currentResult.case_id)
                ...
                currentResult.comment = currentResult.comment.concat(`\n\nRequest: ${execution.request.method} ${url}`);

                if (urlObject.body) currentResult.comment = currentResult.comment.concat(`\nQuery: ${urlObject.body.raw}`);
