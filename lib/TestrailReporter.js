  ...
  constructor(emitter, reporterOptions, options) {
    ...
    emitter.on('beforeDone', (err, args) => {
      if (results.length > 0) {
        ...
        const customFieldName = process.env.TESTRAIL_CFNAME;
        const customFieldId = process.env.TESTRAIL_CFID;
        ...
        var initial_cases_number = cases.length;
        ...
        // Filter the test cases by custom field ID (it should be an array)
        cases = cases.filter(tc => String(tc["custom_"+customFieldName.toLowerCase()]).includes(customFieldId));

        var caseIds = cases.map(c => c.id);

        // Include all test cases into the test run if our filter returned zero results
        var addRunPayload = {};
        if (cases.length < initial_cases_number) {
          addRunPayload = {
            name: title,
            suite_id: suiteId,
            include_all: false,
