...
class TestRailReporter {
  ...
  onComplete(err, args) {
    ...
    if (!missingOption) {
      ...
      this.pass_id = process.env.TESTRAIL_PASSED_ID || 1;
      this.fail_id = process.env.TESTRAIL_FAILED_ID || 5;
      this.skipped_id = process.env.TESTRAIL_SKIPPED_ID || 4;
      ...
      execution.assertions.forEach((assertion) => {
        ...
        while (i < strings.length && strings[i].match(testCaseRegex)) {
          ...
          if (assertion.skipped === true) {
            ...
            } else currentResult.status_id = this.pass_id;

            if (this.logging.toLowerCase() === 'none')
