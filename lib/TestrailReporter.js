            ...
            if (matches) {
              ...
              if (assertion.skipped === true) {
                ...
                if (execution.request.body) {
                  const body = execution.request.body.raw;
                  if (
                    execution.request.headers.find(
                      (header) =>
                ...
                if (execution.response) {
                  ...
                  currentResult.comment = `A request error has occurred.\n${JSON.stringify(
              ...
              if (process.env.TESTRAIL_VERSION)
                currentResult.version = process.env.TESTRAIL_VERSION;
  ...
  pushToTestrail() {
    if (this.results.length > 0) {
      ...
      if (!this.title) {
        ...
        this.title =
          process.env.TESTRAIL_TITLE ||
          `${JSON.parse(response.getBody()).name}: Automated Test Run`;
      }
